...
- `guard_project_addon_commercial_freeze()` — distinct SQL identifier from the `projects`-table trigger `guard_project_commercial_freeze()` (Postgres cannot host two zero-argument functions with the identical name in one schema; approved naming resolution, Task 002 revision round 2) — `BEFORE INSERT OR UPDATE ON project_addons FOR EACH ROW`: looks up `projects.payment_status` for `NEW.project_id` **using `SELECT ... FOR UPDATE`** (Task 002 revision round 4); if `= 'PAID'`, raises an exception; if no parent Project row is found, fails closed with an explicit exception rather than proceeding with an unknown status. This blocks adding **and** revoking add-ons once paid — so once `PAID`, even the revocation trio becomes frozen (in addition to the identity/snapshot columns already frozen unconditionally by the trigger above). **Commercial addon mutations for one Project are serialized by locking the parent `projects` row before mutation:** the row lock is held for the rest of the transaction, so (1) concurrent add-on mutations for the same Project serialize against each other before `sync_project_commercial_totals()` computes its SUM, preventing a lost-update/stale-total race, and (2) an add-on mutation and the `UNPAID → PAID` transition on the same Project serialize against each other — whichever acquires the row lock first completes (or is rejected) before the other proceeds.
- `guard_project_addon_revocation_immutability()` — new, Task 002 revision round 2 — `BEFORE UPDATE ON project_addons FOR EACH ROW`: once `revoked_at` first becomes non-`NULL`, freezes the entire revocation record (`revoked_at`, `revoked_by`, `revoked_reason`) — raises an exception if any of the three differ from `OLD` while `OLD.revoked_at IS NOT NULL`. This makes revocation monotonic: `NULL → non-NULL` (first revocation) remains allowed while the row is still active and the parent Project is `UNPAID`; `non-NULL → NULL` ("un-revoking") and `non-NULL → a different value` (rewriting revocation history) are both forbidden unconditionally, for every normal code path. If the customer wants the same add-on again after revocation while still `UNPAID`, application code creates a **new** `project_addons` row (a fresh purchase snapshot) — this guard is what makes that the only possible path, closing a gap where the original design allowed `revoked_at` to be reset to `NULL`, effectively resurrecting an old snapshot in place.
- `sync_project_commercial_totals()` — `AFTER INSERT OR UPDATE ON project_addons FOR EACH ROW` — **[R8]**: recomputes `SELECT COALESCE(SUM(price_vnd_snapshot), 0) FROM project_addons WHERE project_id = NEW.project_id AND revoked_at IS NULL` and writes it to `projects.addon_total_vnd`, then sets `projects.total_price_vnd = projects.base_price_vnd + projects.addon_total_vnd`, in the same statement/transaction as the triggering `project_addons` write. Because the guard triggers above always run first and block the write entirely when the project is `PAID`, this sync trigger only ever executes while `UNPAID` — the triggers compose correctly by construction, not by coincidence.
...
RLS: enabled + forced. SELECT: `is_staff()`. INSERT: `is_staff()` (subject to the freeze trigger). UPDATE: `is_staff()` — **[F9]**: the RLS policy itself still just grants "staff may attempt an UPDATE"; which columns actually succeed is now enforced by `guard_project_addon_identity_immutability()` (identity/snapshot columns, always), `guard_project_addon_revocation_immutability()` (the revocation trio, monotonic — see above), and `guard_project_addon_commercial_freeze()` (blocks the revocation trio entirely once `PAID`), not by convention. DELETE: no policy for any role — revoke, never delete. Anonymous/guest/customer token: none.
...
- **Sales-flow mutability, resolved (§R7):** package/add-on selection may change freely while `projects.payment_status = 'UNPAID'` — this is normal, expected staff correction of an in-progress sale, not a destructive edit. The moment `payment_status` transitions to `'PAID'`, `guard_project_commercial_freeze()` (on `projects`) and its sibling `guard_project_addon_commercial_freeze()` on `project_addons` (§2.3, §2.6) make `service_package_id`, the three package snapshot/price columns, and all `project_addons` rows immutable for every normal code path. `CHECK ((payment_status='PAID') = (paid_at IS NOT NULL))` keeps that pair of columns consistent.
...
- **Storage cleanup order — [R15]:** there is no atomic transaction spanning Postgres and Supabase Storage, and this plan does not claim otherwise. When a `project_media` row is genuinely deletable (no incoming RESTRICT-FK reference — `invitation_version_media`, or either `wedding_details.groom_bank_qr_media_id`/`bride_bank_qr_media_id` composite FK), the server deletes the **database row first**, then the **Storage object second**. If the Storage deletion step fails or crashes after the DB commit, the result is an orphaned Storage object — a safe failure mode (wasted storage, cleanable later by a maintenance job) — rather than the reverse order's failure mode (a dangling DB reference to already-missing storage, which could be treated as still-valid by some other code path). Preferring an orphaned object over a broken published invitation is the explicit design intent.
...
0006_project_addons.sql
    ...
    - guard_project_addon_identity_immutability() ([F9], new — freezes
      ...
      unconditionally).
    - guard_project_addon_revocation_immutability() (new, Task 002 revision
      round 2 — freezes revoked_at/revoked_by/revoked_reason once
      revoked_at first becomes non-NULL; makes revocation monotonic).
    - guard_project_addon_commercial_freeze() (addons side; distinct name
      from projects-side guard_project_commercial_freeze() — approved
      naming resolution, Task 002 revision round 2; locks the parent
      projects row via SELECT ... FOR UPDATE and fails closed if not
      found — Task 002 revision round 4) + sync_project_commercial_totals()
      trigger.
